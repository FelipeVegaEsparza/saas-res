...
<div class="mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-3">
            <div style="width: 50px; height: 50px; background: {{ $area->color }}; border-radius: 10px; display: flex; align-items: center; justify-content: center;">
                <i class="ri {{ $area->icon }} ri-24px text-white"></i>
            </div>
            <div>
                <h1 class="mb-0">{{ $area->name }}</h1>
                <p class="text-muted mb-0">
                    {{ $ordersByStatus['pending']->count() + $ordersByStatus['preparing']->count() + $ordersByStatus['ready']->count() }} pedidos activos
                </p>
            </div>
        </div>
        <div>
        ...
        <div class="kds-column">
            <div class="column-header">
                <h5 class="mb-0 d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-secondary">{{ $ordersByStatus['pending']->count() }}</span>
                </h5>
            </div>

            @forelse($ordersByStatus['pending'] as $order)
                @include('tenant.kds.partials.order-card', ['order' => $order, 'area' => $area])
            @empty
        ...
        <div class="kds-column preparing">
            <div class="column-header preparing">
                <h5 class="mb-0 d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-warning">{{ $ordersByStatus['preparing']->count() }}</span>
                </h5>
            </div>
        ...
        <div class="kds-column ready">
            <div class="column-header ready">
                <h5 class="mb-0 d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-success">{{ $ordersByStatus['ready']->count() }}</span>
                </h5>
            </div>
