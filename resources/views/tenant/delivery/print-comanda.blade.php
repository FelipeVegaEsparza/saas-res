...
<head>
    <meta charset="UTF-8">
    <title>Comanda - {{ $deliveryOrder->order_number }}</title>
    <style>
        @page {
            size: 80mm auto;
            margin: 0;
...
<body>
    <div class="header">
        ...
        <div class="order-type">
            {{ $deliveryOrder->type === 'delivery' ? '🚴 DELIVERY' : '🛍️ PARA LLEVAR' }}
        </div>
    </div>
    ...
    <div class="info-section">
        <div class="info-row">
            ...
            <span>{{ $deliveryOrder->order_number }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Hora:</span>
            <span>{{ $deliveryOrder->created_at->format('d/m/Y H:i') }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Estado:</span>
            <span>{{ $deliveryOrder->status_label }}</span>
        </div>
    </div>
    ...
    <div class="customer-section">
        ...
        <div class="info-row">
            ...
            <span>{{ $deliveryOrder->customer_phone }}</span>
        </div>
        @if($deliveryOrder->customer_email)
            <div class="info-row">
                <span class="info-label">Email:</span>
                <span style="font-size: 10px;">{{ $deliveryOrder->customer_email }}</span>
            </div>
        @endif
    </div>

    @if($deliveryOrder->type === 'delivery' && $deliveryOrder->delivery_address)
        <div class="address-section">
            <div class="address-title">📍 DIRECCIÓN DE ENTREGA:</div>
            <div>{{ $deliveryOrder->delivery_address }}</div>
            @if($deliveryOrder->delivery_zone)
                <div style="margin-top: 5px;">
                    <span class="info-label">Zona:</span> {{ $deliveryOrder->delivery_zone }}
                </div>
            @endif
        </div>
    @endif
    ...
    <div class="total-section">
        <div class="total-row">
            <span>Subtotal:</span>
            <span>${{ number_format($deliveryOrder->subtotal, 0, ',', '.') }}</span>
        </div>
        @if($deliveryOrder->delivery_fee > 0)
            <div class="total-row">
                <span>Envío:</span>
                <span>${{ number_format($deliveryOrder->delivery_fee, 0, ',', '.') }}</span>
            </div>
        @endif
        <div class="total-row total-final">
            <span>TOTAL:</span>
            <span>${{ number_format($deliveryOrder->total, 0, ',', '.') }}</span>
        </div>
    </div>
