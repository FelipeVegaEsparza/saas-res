...
@section('title', 'Editar Tutorial - Admin')

@section('content')
<div class="mb-4">
    <h1 class="mb-1">Editar Tutorial</h1>
    <p class="text-muted">Modifica la información del tutorial</p>
</div>
                ...
                <form action="{{ route('admin.tutorials.update', $tutorial->id) }}" method="POST">
                        ...
                        <select class="form-select @error('tutorial_category_id') is-invalid @enderror"
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}"
                                        {{ old('tutorial_category_id', $tutorial->tutorial_category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                               id="title" name="title" value="{{ old('title', $tutorial->title) }}" required>
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  id="description" name="description" rows="3">{{ old('description', $tutorial->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control @error('youtube_url') is-invalid @enderror"
                               id="youtube_url" name="youtube_url" value="{{ old('youtube_url', $tutorial->youtube_url) }}"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('order') is-invalid @enderror"
                               id="order" name="order" value="{{ old('order', $tutorial->order) }}" min="0">
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="ri ri-save-line me-1"></i>Actualizar Tutorial
                        </button>
